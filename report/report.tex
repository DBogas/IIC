...
\end{figure}
\begin{comment}
There are still two types of grouping to explore. Using the source files of the address grouping and displaying the graph according to geographical coordinates, the graph generated resembles the one we saw above, but with less nodes and edges. This graph's nodes were also colored, according to the number of stops they have. This tells us that 31.25$%$ of the streets have only one stop, and a quarter of the network has 2 stops. That's normal, but what is not normal is for one street to hold 86 bus stops! This is because we are talking about Estrada da Circunvalação, that is about 17km long. Exploring the Data Laboratory, and looking at the closeness centrality, we see that the. highest value(the node that is closer to all the others) is Estrada da Circunvalação, which seems normal. In fact, this street is so long, that it also tops the harmonic and betweeness centrality columns. The betweeness centrality tells us the number of times this node was a path of a shortest path between two other nodes. The harmonic part, that's still up for research.

ver o que é a harmonic closeness centrality.
\end{comment}
